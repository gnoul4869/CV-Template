# Personal Website
 Personal Website built using only HTML based on these requirements:

```
This website consists of at least the following pages:
...
→ Links (must have: links from one page to another page, links to external websites, and links from one part of a page to 
another part of the page, e.g. links to go to the top of the page)
...
→ Forms (ensure that "method" and "action" attributes are set, and "enctype" need to be set if required;
use different input fields (at least 4 different types) like text, email, phone, radio, select, textarea, etc;
use labels for accessibility, set element names properly)
